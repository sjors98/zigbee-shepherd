...
handlers.devIncoming = function (devInfo, resolve) {
    ...
    var processDev = Q.fcall(function () {
        ...
        if (accepted) {
            ...
            return self.remove(dev.getIeeeAddr(), { reJoin: false }).then(function () {
                return Q.ninvoke(self._devbox, 'remove', dev._getId());
            });
        }
    }).fail(function (err) {
        self.emit('error', err);
...
handlers.leaveInd = function (msg) {
    ...
    if (dev) {
        ...
        return Q.ninvoke(self, '_unregisterDev', dev).then(function() {
            debug.shepherd('Device: %s leave the network.', ieeeAddr);
            self.emit('ind:leaving', epList, ieeeAddr);
        })
    }
};
