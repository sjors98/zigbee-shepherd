...
ZShepherd.prototype.info = function () {
    ...
    return {
        ...
        firmware: {
            transportrev: firmware.transportrev,
            product: firmware.transportrev,
    ...
    if (ep.constructor.name === 'Zive') {
        ...
        ep.read = function (cId, attrId, callback) {
            ...
            if (typeof attrId === 'object') {
                ...
                attrIdVal = attr ? attr.value : attrId;
            }

            self._foundation(ep, ep, cId, 'read', [{ attrId: attrIdVal }]).then(function (readStatusRecsRsp) {
        ...
        ep.report = function (cId, attrId, minInt, maxInt, repChange, callback) {
            ...
            if (cfgRpt) {
                if (typeof attrId === 'object') {
                    attrIdVal = attrId.id;
                    attrTypeVal = zclId.dataType(attrId.type).value;
                } else {
                    var attr = zclId.attr(cId, attrId);
                    attrIdVal = attr ? attr.value : attrId;
